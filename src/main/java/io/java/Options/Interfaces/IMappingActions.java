...
public interface IMappingActions<Source, Destination> {
	/**
	 * Subscribes {@link MappingActionsEnum} actions
	 * @param targetAction
	 * @param action
	 */
	void on(MappingActionsEnum targetAction, IV2Callback<Source, Destination> action);
	 ...
	 */
	void call(MappingActionsEnum targetAction, Source src, Destination dst);
     ...
     */
    void beforeMap(IV2Callback<Source, Destination> modifier);
    
    /**
     * Subscribes {@link MappingActionsEnum.AFTER_MAP} action
     * @param modifier the delegate having the modification
     */
    void afterMap(IV2Callback<Source, Destination> modifier);
}
